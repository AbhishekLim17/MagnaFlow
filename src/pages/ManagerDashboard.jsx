// Manager Dashboard - tasks and staff roster scoped to the manager's own
// project(s). Can create new staff, auto-assigned into the project.
...
const AddStaffDialog = ({ open, onOpenChange, onCreated, orgId, projectId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast({ title: 'Staff account created', description: `${form.name} has been added to your project.` });
...
const ManagerDashboard = () => {
  ...
  const loadStaff = async () => {
    try {
      ...
      const data = await getAllUsers({ role: 'staff', projectIds: user?.projectIds || [] });
      setStaff(data);
    } catch (error) {
      console.error('Error loading project staff:', error);
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-300">{user?.name} · Manager</p>
          </div>
        ...
        <Card className="glass-effect p-6 mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-white">
              <CheckSquare className="text-purple-300" /> Project Tasks ({tasks.length})
            </CardTitle>
          </CardHeader>
          <CardContent>
            {tasksLoading ? (
              ...
              <p className="text-gray-400 text-center py-8">No tasks in your project yet.</p>
            ) : (
        ...
        <Card className="glass-effect p-6 mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-white">
              <GanttChartSquare className="text-purple-300" /> Project Timeline
            </CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="glass-effect p-6">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="flex items-center gap-2 text-white">
              <Users className="text-purple-300" /> Project Staff ({staff.length})
            </CardTitle>
            <Button
          ...
          <CardContent>
            {staffLoading ? (
              ...
              <p className="text-gray-400 text-center py-8">No staff in your project yet.</p>
            ) : (
